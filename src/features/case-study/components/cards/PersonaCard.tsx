...
const PersonaCard: React.FC<CardProps> = ({ image, title, job, age, quote, bio, frustrations, motivations }) => {
  ...
  return (
    <div
      className="relative w-60 h-90 [perspective:800px] group cursor-pointer"
        ...
        {hasHoverContent && (
          ...
          className={`relative z-20 top-0 left-0 p-4 typography transition-all duration-1000 transform ${
            hasHoverContent ? "-translate-y-32 group-hover:translate-y-0" : ""
              ...
              {age && (
            <p className="m-0 pb-4 items-start text-left font-sans font-medium text-neutral-800 tracking-wide leading-cozy hidden group-hover:block text-body2">{age}</p>
          )}
          {quote && (
            <p className="m-0 pb-4 items-start text-left font-sans italic font-thin text-neutral-700 tracking-wide leading-cozy hidden group-hover:block text-body2">{quote}</p>
          )}
          {bio && (
            <p className="m-0 pb-4 items-start text-left font-sans font-thin text-neutral-700 tracking-wide leading-cozy hidden group-hover:block text-body2">{bio}</p>
          )}
              ...
              <>
                {frustrations.map((frustrations, index) => (
                  <div key={index} className="relative flex justify-start gap-3 items-start">
                    ...
                    <p className="m-0 w-full pb-4 pt-0.5 items-start text-left font-sans font-thin text-neutral-700 tracking-wide leading-cozy hidden group-hover:block text-body2">{frustrations}</p>
                  </div>
                ))}
              </>
            )}
              ...
              <>
                {motivations.map((motivations, index) => (
                  <div key={index} className="relative flex justify-start gap-3 items-start">
                    ...
                    <p className="m-0 w-full pb-4 pt-0.5 items-start text-left font-sans font-thin text-neutral-700 tracking-wide leading-cozy hidden group-hover:block text-body2">{motivations}</p>
                  </div>
                ))}
              </>
            )}
          ...
          <div className={`absolute z-20 top-0 left-0 p-4 transition-all duration-1000 transform ${
            ...
            <h3 className="m-0 pb-2  items-start text-left font-sans font-medium text-primary tracking-wide leading-cozy text-2xl">{title}</h3>
            <p className="m-0  text-neutral-500 font-sans text-left text-body2 font-thin">
              {job}
            </p>
