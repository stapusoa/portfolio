...
const SiteMap: React.FC<MapProps> = ({ pages }) => (
    ...
    <div className="col-start-1 col-span-8 border-solid border border-neutral-200 font-sans bg-white overflow-hidden rounded-lg p-6">
      <ul className="grid grid-cols-10 grid-flow-row m-0 p-0 justify-start gap-4">
      {pages.map((page, pageIndex) => (
        <li key={pageIndex} className="relative list-none col-start-1 col-span-10 md:col-span-5 lg:col-span-2 items-center w-full">
          <span
            className="relative flex items-center bg-primary rounded-md justify-center text-center text-white font-medium text-sm border px-3 py-2 max-w-300"
          >
           {page.page}
            <span
          ...
          {page.sections.map((section, sectionIndex) => (
            ...
            <li key={sectionIndex} className="relative list-none bg-neutral-600 overflow-hidden rounded-md border-0 my-4">
              ...
              >
                <span className="font-medium text-white text-sm px-3 py-2">
                {section.section}
                </span>
              </label>
              {/* Rotating SVG */}
