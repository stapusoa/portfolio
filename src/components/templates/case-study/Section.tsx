    ...
    <div className={`bg-transparent mx-auto max-w-300 relative z-32 pt-16 pb-12 ${containerPadding} sm:${containerPadding} md:px-14 lg:px-32 grid grid-cols-8 gap-8 items-start`}>
      ...
      {title?.title && (
        <div className={`py-2 text-left ${title.titleFull ? "col-span-8" : "col-span-8 md:col-span-4"} ${siblingPadding}`}>
          <h2 className="text-3xl pb-8 font-semibold tracking-wide">{title.title}</h2>
        </div>
      )}
      ...
      {isGroupedLeft ? (
        <>
          ...
          <div className="col-span-8 md:col-span-4 flex flex-col gap-4">
            {text?.text && (
              <div className="m-0 p-0 font-sans text-neutral-600 text-left font-light text-2xl leading-relaxed">
            ...
            {image?.image && (
              <div className="w-full">
                {image.image.map((img, index) => (
                  <img loading="lazy" decoding="async" key={index} src={img} alt={`Section Image ${index}`} className="w-full h-auto rounded-lg mt-2" />
                ))}
              </div>
            )}
            ...
            <div className="col-span-8 md:col-span-4 flex flex-col gap-8">
              {keynotes.map((keynote, index) => (
                <div key={index} className="grid gap-2">
                  <div className="m-0 text-left font-sans font-light text-2xl text-neutral-600 tracking-wide">
                    {keynote.label}
                  </div>
                  <div className="m-0 text-left font-sans text-neutral-600 font-light text-2xl leading-relaxed">
                    {keynote.details}
                  </div>
                </div>
              ))}
        ...
        <>
          ...
          {text?.text && (
            <div
              className={`m-0 p-0 font-sans text-neutral-600 text-left font-light text-2xl leading-relaxed 
  ...
  >
    {image.image.map((img, index) => (
      <div key={index} className="snap-start flex items-start shrink-0 w-full md:w-auto">
        <img loading="lazy" decoding="async" src={img} alt={`Section Image ${index}`} className="w-full h-auto rounded-lg" />
      </div>
    ))}
  </div>
)}
